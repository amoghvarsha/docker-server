...
function InitEnv() {
    ...
    # RESTORE_FILE_DATE_FORMAT (Optional)
    RESTORE_FILE_DATE="${RESTORE_FILE_DATE:-"%Y%m%d"}"
    RESTORE_FILE_DATE_FORMAT=$(echo "${RESTORE_FILE_DATE}${RESTORE_FILE_DATE_SUFFIX}" | sed 's/[^0-9a-zA-Z%_-]//g')
    ...
    Echo Y "RESTORE_FILE_DATE_FORMAT : ${RESTORE_FILE_DATE_FORMAT}"
